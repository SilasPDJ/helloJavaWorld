...
public class desafioPrePascoa {
	...
	public static void main(String[] args) {
		...
		separatorPrint("~", 60);
		ex3();
//		ex3("Lotofácil", 25, 15, 20);
	}

	public static void ex3(String jogoNome, int valMax, int lotoQtdNums, int lotoQtdMaxNums) {

		int gameMinNum, gameMaxNum;
		gameMinNum = 1;
		gameMaxNum = valMax;

		int[] senaResult = getSenaSorteio(lotoQtdNums, gameMinNum, gameMaxNum, true);
		int betNumSize = 6;
		Scanner input = new Scanner(System.in);

		// TAMANHO DA APOSTA
		separatorPrint("_~" + jogoNome + "~_", 6);
		separatorPrint("~", 60);
		System.out
				.print("Quantos números deseja utilizar? Escolha entre " + lotoQtdNums + " e " + lotoQtdMaxNums + ": ");
		betNumSize = input.nextInt();
		if (betNumSize >= lotoQtdNums && betNumSize <= lotoQtdMaxNums) {
		...
		if (automatizaSorteio == false) {
			for (int i = 0; i < betNumSize; i++) {
				valJaJogado = true;
				while (jogadaArr[i] < gameMinNum || jogadaArr[i] > gameMaxNum || valJaJogado) {

					System.out.printf("Digite o %2dº número: ", i + 1);
					jogadaArr[i] = input.nextInt();
					// cv = check valor
					for (int cv = 0; cv < betNumSize; cv++) {
						if (cv != i) {
							if (jogadaArr[cv] == jogadaArr[i]) {
								System.out.println("Valor repetido. Tente outro.");
								valJaJogado = true;
								break;
							} //
							else {
		...
		// Acertos
		System.out.printf("Houve %02d acertos", contAcertos);
	...
	protected static int[] getSenaSorteio(int tam, int min, int max, boolean semRepetir) {
		int i, r;
		int val;
		int[] arr = new int[tam];
